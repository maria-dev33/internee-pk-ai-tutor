...
export default function HomePage() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#0a0f1e',fontFamily:'sans-serif',color:'white'}}>
      ...
      {/* Navbar */}
      <nav style={{borderBottom:'1px solid #1e2a45',padding:'16px 40px',display:'flex',alignItems:'center',justifyContent:'space-between',background:'rgba(10,15,30,0.95)',position:'sticky',top:0,zIndex:100}}>
        ...
        <div style={{display:'flex',gap:'32px',fontSize:'14px'}}>
          <a href="/" style={{color:'#06b6d4',textDecoration:'none'}}>Home</a>
          <a href="#modules" style={{color:'#94a3b8',textDecoration:'none'}}>Modules</a>
        </div>
        <a href="#modules" style={{background:'linear-gradient(135deg,#06b6d4,#8b5cf6)',padding:'8px 20px',borderRadius:'8px',fontSize:'14px',fontWeight:'600',cursor:'pointer',textDecoration:'none',color:'white'}}>
          Get Started
        </a>
      </nav>
      ...
      <div style={{textAlign:'center',padding:'80px 24px 60px',position:'relative'}}>
        ...
        <h1 style={{fontSize:'52px',fontWeight:'800',margin:'0 0 16px',lineHeight:1.1}}>
          Build Skills.<br/>
        ...
        {/* ONE button only */}
        <a href="#modules" style={{display:'inline-block',background:'linear-gradient(135deg,#06b6d4,#8b5cf6)',padding:'14px 36px',borderRadius:'10px',fontWeight:'700',fontSize:'16px',cursor:'pointer',textDecoration:'none',color:'white'}}>
          Start Learning →
        </a>
      ...
      {/* Modules */}
      <div id="modules" style={{maxWidth:'1100px',margin:'0 auto',padding:'0 24px 80px'}}>
          ...
          {LEARNING_MODULES.map((mod) => {
            ...
            return (
              <a key={mod.id} href={`/tutor/${mod.id}`} style={{textDecoration:'none',color:'white'}}>
                <div style={{background:'#0d1526',border:'1px solid #1e2a45',borderRadius:'16px',padding:'24px',cursor:'pointer',transition:'border 0.2s'}}
                ...
                >
                  ...
                  <div style={{height:'4px',background:'#1e2a45',borderRadius:'99px',overflow:'hidden'}}>
                    <div style={{height:'100%',background:'linear-gradient(90deg,#06b6d4,#8b5cf6)',width:`${progress}%`,borderRadius:'99px'}}/>
                  </div>
                </div>
              </a>
            );
